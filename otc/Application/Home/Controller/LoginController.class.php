...
//            $this->success('登入成功');
    ...
    public function find_password()
    {
        if (IS_AJAX) {
            ...
            $content = '【场外】您正在重置登录密码，验证码：'.$code.'。10分钟内有效。';
	...
	public function find_password_email()
    {
        if (IS_AJAX) {
            ...
            $content = '【18USDT】您正在重置登录密码，验证码：'.$code.'。10分钟内有效。';
            
			$result = send_email($mobile,'重置登录密码',$content);
            if (!$result['error']) {
                $data['status'] = 1;
                $data['content'] = '短信验证码已发送到你的手机，请查收';
                $this->ajaxReturn($data);
