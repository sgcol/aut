...
class OrderController extends HomeController
{
        ...
        if (IS_AJAX) {
            ...
            if(!$ad_type) {
                ...
                if ($amount<$adinfo['min_amount'] || $amount>$max_amount) {
                    $this->error("请在交易限额范围内下单，交易限额: {$adinfo['min_amount']}-{$max_amount} {$adinfo['currency']}");
                }
            } else {
                if ($amount<$adinfo['min_amount'] ) {
                    $this->error("最低需要10CNY");
                }
            }
    ...
    public function detail()
    {
        ...
        switch ($orderinfo['status']) {
            case 0:
                $buy_stage_text='已拍下';
                $buy_status_decs='订单将在托管中保存<span class="blue-text expiration">'.$orderinfo['delaytime'].'</span>分钟, 逾期未付款将自动取消';
                $sell_stage_text='已拍下';
                $sell_status_decs='订单将在托管中保存<span class="blue-text expiration">'.$orderinfo['delaytime'].'</span>分钟, 逾期未付款将自动取消';
        ...
        /*付款信息*/
        $fkfs = array(1=>'bank',2=>'alipay',3=>'weixin');
        ...
        $data = array(
            ...
            'paymethod' => $fkfs1[$orderinfo['fkfs']],
        );
        $url.= '/merchant/'.$orderinfo['sellid'].'/onCreateOrder';
        ...
        $re  =  json_decode(request_post($url, $data2),1);
     
        $fkinfo = M('user_bank')->where(array('userid'=>$orderinfo['sellid'],'type'=>$fkfs[$orderinfo['fkfs']]))->find();
		
		if($re['result'] == 'ok') {
			$fkinfo['is_img'] = 0;
			$fkinfo['erweima'] = $re['url'];
		} else {
			$fkinfo['is_img'] = 1;
		}

        $this->assign('fkinfo', $fkinfo);
